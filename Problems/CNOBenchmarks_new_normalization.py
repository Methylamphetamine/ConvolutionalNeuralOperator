...
class WaveEquationDataset(Dataset):
    def __init__(self, which="training", nf=0, training_samples = 512, t = 5, s = 64, in_dist = True):
        ...
        self.reader = h5py.File(self.file_data, 'r')
        self.normalization_reader = h5py.File(data_folder + "WaveData_64x64_IN.h5", 'r')
        
        #Load normaliation constants:
        self.min_data = self.normalization_reader['min_u0'][()]
        self.max_data = self.normalization_reader['max_u0'][()]
        self.min_model = self.normalization_reader['min_u'][()]
        self.max_model = self.normalization_reader['max_u'][()]
...
class AllenCahnDataset(Dataset):
    def __init__(self, which="training", nf = 0, training_samples = 256, s=64, in_dist = True):
        ...
        self.reader = h5py.File(self.file_data, 'r')
        self.normalization_reader = h5py.File(data_folder + "AllenCahn_64x64_IN.h5", 'r')
...
class DarcyDataset(Dataset):
    def __init__(self, which="training", nf=0, training_samples=256, insample=True):
        ...
        self.reader = h5py.File(self.file_data, 'r')
        self.normalization_reader = h5py.File(data_folder + "Darcy_64x64_IN.h5", 'r')
